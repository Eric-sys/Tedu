...
# date01 = StudentModel("悟空", 23, 96)
# controller.add_student(date01)
# controller.add_student(StudentModel("八戒", 25, 65))
# print(controller.remove_student(1000))
#
...
class StudentManagerView:
    ...
    def __output_students(self):
        for item in self.__controller.stu_list:
            print("编号是：%d,姓名是%s,年龄是%d,成绩是%d." % (int(item.id), item.name, int(item.age), int(item.score)))
